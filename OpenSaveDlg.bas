...
Public Function Save_File(hWnd As Long) As String
   ...
   If (sv&) Then
      ...
      If Dir(temp) <> Empty And MsgBox("File already exists. Do you wish to over write it?" & vbNewLine & temp, vbYesNo, "File exists") = vbNo Then temp = ""
      Save_File = temp
   End If
End Function
...
Public Sub InitOpenDlg()
   With OpenFileDialog
     ...
     .hwndOwner = hWnd&
     .hInstance = App.hInstance
     .lpstrFilter = "Saved Web Page" + Chr$(0) + "*.htm;*.htm?"
     .lpstrFile = Space$(254)
     .nMaxFile = 255
     .lpstrFileTitle = Space$(254)
     .nMaxFileTitle = 255
     .lpstrInitialDir = CurDir
     .lpstrTitle = "Load Page"
     .flags = 0
   End With
End Sub
Public Sub InitDlgs()
